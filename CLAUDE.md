...
本节是项目最高优先级红线，绝对优先于任何放行（承 §13）。任一 STOP-* 触发时，执行方（Claude Code）立即停止、贴当前进度 + 触发原因 + 建议下一步，等主理人在 chat 显式放行；不得自行续跑、不得"为跑通"绕过。这六条触发时 Plan-Mode 不适用（§7.1），强制走逐项放行；STOP 永远先于 Plan（§7.5）；实际副作用超出 [PLAN] 声明范围即属中断（§7.4）。红线靠文字约束，更靠三闸物理背板（GATE-1 / GATE-2 / GATE-3，见 §6.6–§6.8）：红线说"不该"，闸保证"不能"。

### §4.1 STOP-B（发单 / 实盘）
触发：任何真实下单、testnet / mainnet 连接、实盘相关 dry-run 校验（Category B）。
动作：只由主理人手敲；执行方不得自主发单。
强制：GATE-2 拦 mainnet 显式误触、GATE-3 拦 hermes trade-live 子命令（§6.8）；能力一出现即被拦。

### §4.2 STOP-DOCS（写治理 / 交接文档）
触发：写 docs/handoffs/*.md 或 CLAUDE.md。
动作：预览先行全流程，顺序不可压并、不可跳——执行方干跑预览（cp 现网为底 + 拼接）→ 回传预览全文 + md5 + wc + 污染探针 → 复核方过眼 → 主理人 chat 显式 go → 才写入 → 写后 md5 / wc / grep 自校。"内容已给" / "规格已定" 绝不充当 go；PHASE 2 必须 cp 已过眼的 /tmp 预览，禁 Edit 重做（否则毁掉"预览==写入"坐实）。承 §6.5 STOP-DOCS-SKIP。
强制：无专用闸；靠 GATE-1 兜底——预览 / 写入可由 CC 跑，落 git 只能主理人手敲。

### §4.3 STOP-ARCH（动 ce9 受保护文件 / 架构改动）
触发：修改 §8.1 列出的 ce9 五保护文件，或架构级改动。
动作：须主理人显式 carve-out 授权（指明范围）+ [PLAN] 过眼 + go；无授权零碰；改后同步迁移 §8.2 基线 md5。
强制：五基线 md5 哨兵（§8.2）——未授权改动令 test_ce9 失败。

### §4.4 STOP-VERDICT（判据异议 / 断言变更）
触发：执行方认为判据有误 / 不可达 / 自相矛盾，或需变更既有测试断言。
动作：显式申报并 STOP 回主理人；禁自行新增 / 删除 / 放宽 / 重新解释判据，禁"为跑通"私改。承 §6.2 四类放水、§6.4 双人分离、教训 CB1。
强制：无闸；靠复核方逐轮核"四类放水零命中"。

### §4.5 STOP-NET（外网 / 离机 I/O）
触发：发起外网调用、连交易所、任何离机 I/O。
动作：须主理人授权；[PLAN] 自检常项"无外网调用 / testnet 连接"。
强制：无闸；靠 [PLAN] 声明 + 复核。

### §4.6 STOP-SYS（系统 / 环境 / 持久化配置改动）
触发：改 pip install / .env / .gitignore、系统或安全设置、持久化配置（如 systemd 守护，M4 阶段）。
动作：须主理人授权；[PLAN] 自检常项"无 pip install / .env / .gitignore 改动"。
强制：无闸；靠 [PLAN] 声明 + 复核。

## §5 三方协作模型

本项目由三方协作，职责严格分离（verdict 出题/达标分离的总原则见 §12，终审归属见 §13）。

### §5.1 角色与职责
| 角色 | 是谁 | 职责 | 禁止 |
|---|---|---|---|
| 主理人 | 项目创始人（Z / Iris） | 唯一授权者：下 go、手敲全部 git 写、授权 testnet/live、终审拍板 | （无） |
| 复核方 | 网页 Claude | 出判据、逐条核 raw 证据、签 CLEAN/驳回、STOP-DOCS 预览过眼 | 不执行、不 commit、不碰机器 |
| 执行方 | Claude Code（机上） | 收 PLAN + 显式 go 后实现/取证/回传 raw | 被 GATE 物理挡 git 写与自主发单 |

### §5.2 git 写权归属
所有 git 写（add / commit / push / reset / ...）只由主理人手敲。执行方被 GATE-1（PreToolUse hook）物理拦截，无法自主 commit（见 §6.6）；复核方从不执行 git。

### §5.3 闸的物理强制
三方纪律靠文字约束，更靠三闸物理背板：GATE-1（git 写）/ GATE-2（mainnet）/ GATE-3（trade-live 发单）。文字红线（§4 STOP-*）与配置级闸互为表里：红线说"不该"，闸保证"不能"（细节见 §6.6–§6.8）。

### §5.4 证据纪律（raw 优于 verdict）
复核方签字前必取原始终端输出（marker 包裹），不接受执行方的自评 / 摘要 / 表格充当证据；md5 由主理人独立核对后方可 commit（细则见 §6.4 / §10.5）。
